...
class PGAWC_Adyen_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_action('woocommerce_api_wc_adyen_redirect', [$this, 'handle_sca_redirect']);
	}
	...
	public function process_payment( $order_id ) {
		...
		if ($result['resultCode'] === 'RedirectShopper') {
			if (isset($result['action']) && $result['action']['type'] === 'redirect') {
				update_option('psawg_redirect_object_' . $order_id, $result['action']);
				return  [
					'result'   => 'success',
	...
	public function receipt_page( $order_id ) {
		$action = get_option('psawg_redirect_object_' . $order_id);
		if (!isset($action['url'])) {
			return;
		}
		...
		echo '<form action="' . $action['url'] . '" method="post" id="psawg_payment_form">';
		foreach ($action['data'] as $key => $value) {
			if ($key === 'TermUrl') $value = $return_url;
	...
	public function handle_sca_redirect()
	{
		...
		$adyen = new PGAWC_Adyen( $this->adyenConfig );
		$payment_details = $adyen->get_payment_details($md, $paRes, $action['paymentData']);
		...
		// payment went through
		update_post_meta( (int) $order_id, '_woocommerce_adyen_payment_data', $payment_details );

		$order->payment_complete();
		$order->add_order_note( sprintf( __( 'Payment was complete via Adyen (PSP Reference: %s)', 'payment-gateway-for-adyen-and-woocommerce' ), $adyen->get_adyen_transaction_url($payment_details['pspReference']) ));
